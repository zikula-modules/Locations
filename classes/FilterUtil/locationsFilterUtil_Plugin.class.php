...
class locationsFilterUtil_Plugin extends locationsFilterUtil_Common
{
     ...
     * @return object locationsFilterUtil_Plugin object (optional) (default: null)
     */
    ...
    public function loadPlugin($name, $config = array())
    {
        ...
        $class = 'locationsFilterUtil_Plugin_' . $name;

        Loader::loadClass($class, LOCATIONS_FILTERUTIL_CLASS_PATH . '/plugins');
    ...
    {
        $obj =& $this->plg[$k];
        if (is_subclass_of($obj, 'locationsFilterUtil_OpCommon')) {
        ...
        if (is_subclass_of($obj, 'locationsFilterUtil_ReplaceCommon')) {
            $this->replaces[] = $k;
        }
    }
    ...
    {
        if (isset($this->plg[$name]) && is_a($this->plg[$name], 'locationsFilterUtil_Plugin_'.$name)) {
            return true;
        }
        return false;
    }
